...
-- Main frame with animated gradient
local hub = Instance.new("Frame")
hub.Name = "MainHub"
hub.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
hub.BackgroundTransparency = 0.35   -- semi-transparent so gradient shows nicely
...
gradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0.00, Color3.fromRGB(140,  80, 220)),
    ColorSequenceKeypoint.new(0.20, Color3.fromRGB( 90, 140, 255)),
    ColorSequenceKeypoint.new(0.40, Color3.fromRGB( 60, 220, 200)),
    ColorSequenceKeypoint.new(0.60, Color3.fromRGB(255, 160, 140)),
    ColorSequenceKeypoint.new(0.80, Color3.fromRGB(220, 100, 180)),
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(160,  90, 240))
})
...
RunService.Heartbeat:Connect(function(deltaTime)
    timePassed = timePassed + deltaTime * 0.18   -- adjust multiplier for speed (smaller = slower)
    local offset = (math.sin(timePassed) * 0.4 + math.cos(timePassed * 1.3) * 0.2)
    gradient.Offset = Vector2.new(offset, offset * 0.7)
end)
...
-- ──────────────────────────────────────────────────────────────────────────────
-- Wallhop logic, input handling, connections, etc. (unchanged)
-- ──────────────────────────────────────────────────────────────────────────────
